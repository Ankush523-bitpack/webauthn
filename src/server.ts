...
export async function verifyRegistration(
  ...
  if (await isNotValid(expected.origin, registration.client.origin))
    throw new Error(
      `Unexpected ClientData origin: ${registration.client.origin}`
    );

  if (await isNotValid(expected.challenge, registration.client.challenge))
    throw new Error(
      `Unexpected ClientData challenge: ${registration.client.challenge}`
    );
...
export async function verifyAuthentication(
  ...
  if (authenticationRaw.credentialId !== credential.id)
    throw new Error(
      `Credential ID mismatch: ${authenticationRaw.credentialId} vs ${credential.id}`
    );
  ...
  if (authentication.client.type !== "webauthn.get")
    throw new Error(
      `Unexpected clientData type: ${authentication.client.type}`
    );

  if (await isNotValid(expected.origin, authentication.client.origin))
    throw new Error(
      `Unexpected ClientData origin: ${authentication.client.origin}`
    );

  if (await isNotValid(expected.challenge, authentication.client.challenge))
    throw new Error(
      `Unexpected ClientData challenge: ${authentication.client.challenge}`
    );
  ...
  const expectedRpIdHash = utils.toBase64url(
    await utils.sha256(utils.toBuffer(rpId))
  );
  if (authentication.authenticator.rpIdHash !== expectedRpIdHash)
    throw new Error(
      `Unexpected RpIdHash: ${authentication.authenticator.rpIdHash} vs ${expectedRpIdHash}`
    );
  ...
  if (authentication.authenticator.counter <= expected.counter)
    throw new Error(
      `Unexpected authenticator counter: ${authentication.authenticator.counter} (should be > ${expected.counter})`
    );
...
async function parseCryptoKey(
  ...
  return crypto.webcrypto.subtle.importKey("spki", buffer, algoParams, false, [
    "verify",
  ]);
}
